...
12. `/admin/trash` 是回收站，用于恢复已删除工单和嵌入页面，或在确认后永久删除。
13. `/admin/cleanup` 是测试数据清理页，必须先预览匹配工单，再确认批量移入回收站。
14. `/admin/settings`、`/admin/account`、`/admin/system` 是登录后可见的占位页，用于查看本地配置、账号和运行维护提示。

## 删除、回收站与测试数据清理

工单删除默认是软删除。工单移入回收站后，会从后台工单列表、业务总览统计、门店查询、门店详情、Excel 导出和受保护附件访问中隐藏，但数据库记录、日志和附件文件仍会保留。回收站里可以恢复工单；只有在回收站里二次确认后，才会永久删除工单记录、关联附件记录和对应物理文件。
