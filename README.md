...
- 日历和日期选择器使用 `config/holidays.json` 标记节假日；文件不存在或 JSON 有误时自动忽略节假日配置。法定节假日每年需要人工维护，格式为 `{ "YYYY-MM-DD": "节假日名称" }`。
...
`config/holidays.json` 是可选节假日配置，只影响排班日历样式和节假日快捷选择，不会阻断启动。
