...
后台提供 `/admin/embedded-pages` 管理嵌入 HTML 页面。管理员可以新增页面、替换 HTML 文件、设置导航名称和页面标题，并启用或停用左侧导航入口。启用后的页面会显示在后台左侧“扩展页面”分组，访问路径为 `/admin/embed/{page_key}`。

嵌入 HTML 文件保存到 `data/embedded_pages/`，属于运行数据，不上传 GitHub，也不会被 `git pull` 覆盖。系统代码更新仍然需要 GitHub push、服务器 git pull 和重启服务；嵌入 HTML 更新不需要重启服务，管理员上传或替换 HTML 后，所有后台用户刷新页面即可看到新版。如果其他用户已经打开旧页面，需要刷新浏览器才会看到最新版本。iframe 地址会带版本参数，避免浏览器缓存旧 HTML；未来如果需要自动实时刷新，可以再接入消息提醒或轮询版本号。

上传限制来自 `config/system.json` 的 `max_embedded_html_mb`，默认 5MB。第一版不解析和清洗 HTML 内容，嵌入 HTML 被视为内部可信内容，只允许后台账号上传。不要上传含有外部恶意脚本、账号密码、令牌或敏感数据的 HTML；如果后续要开放给非技术人员使用，需要增加更严格的 HTML 清洗和审核。
...
1. 打开后台管理页。后台采用“止痒 ERP”布局，左侧导航包含业务总览、工单管理、门店查询、配置管理、账号设置和系统设置。数据导出入口统一放在工单管理页右上角。
...
12. `/admin/settings`、`/admin/account`、`/admin/system` 是登录后可见的占位页，用于查看本地配置、账号和运行维护提示。
...
从工单管理页点击导出时，会自动携带当前门店、状态、时效、日期、关键词等筛选条件；导出的是当前筛选结果的全部工单，不受当前分页影响。左侧导航不再单独显示“数据导出”菜单。
...
- `max_embedded_html_mb`：后台嵌入 HTML 单文件大小限制，默认 5MB。
