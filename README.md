...
嵌入页面文件保存到 `data/embedded_pages/{page_key}/`，属于运行数据，不上传 GitHub，也不会被 `git pull` 覆盖。新上传的单 HTML 会保存为 `index.html`；ZIP 会解压到同一个 page_key 目录，根目录必须包含 `index.html`，可包含 `assets/` 下的图片、CSS、JS 等资源。旧版本遗留的 `data/embedded_pages/{page_key}.html` 仍可兼容读取，但新上传统一使用目录结构。系统代码更新仍然需要 GitHub push、服务器 git pull 和重启服务；嵌入页面更新不需要重启服务，管理员上传或替换后，所有后台用户刷新页面即可看到新版。如果其他用户已经打开旧页面，需要刷新浏览器才会看到最新版本。iframe 地址会带版本参数，避免浏览器缓存旧 HTML；未来如果需要自动实时刷新，可以再接入消息提醒或轮询版本号。
...
- `max_embedded_zip_mb`：后台嵌入 ZIP 资源包大小限制，默认 100MB。ZIP 根目录必须包含 `index.html`，可包含 `assets/` 图片、CSS、JS 等资源。
