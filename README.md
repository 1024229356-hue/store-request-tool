...
- 排班支持自定义时间，可填写自定义名称、开始时间、结束时间、休息分钟和工时；手动工时优先，工时留空时按开始/结束时间扣除休息分钟自动计算，结束时间早于开始时间时按跨天班次处理。
- 排班看板展示当月已排工时、截至当前日期已排工时、当月排班人数/人次、休息班次数、自定义/加班班次数、门店工时排行和班次分布。这里统计的是已排计划工时，不是打卡实际工时。
- 同一门店内，同一员工同一天重复保存会更新原记录；跨店员工可以在不同门店保留同一天的排班，员工卡片“查看排班”会进入该员工所有可排门店视图。
- 日历和日期选择器优先使用 `/admin/settings` 中维护的节假日；节假日表为空时才兼容读取 `config/holidays.json`。法定节假日、调休日和公司特殊日可以分别维护样式。
...
11. 后台 `/admin/dashboard` 提供工单运营看板和排班运营看板基础版，默认查看近 7 天，可按今日、近 7 天、本月、上月或自定义日期范围筛选，并支持门店、需求类型、状态、处理人、排班员工、班次、自定义时间和异常排班筛选；工单看板展示工单总览、SLA、门店问题排行、需求类型排行、处理人效率和自动分派效果，排班看板展示排班总览、门店工时排行、员工工时排行、班次分布、异常排班和自定义/加班工时，所有指标都会按当前账号 `data_scope` 自动收敛。
...
16. `/admin/settings` 可维护门店、品牌和节假日；`/admin/account` 维护人员与账号；`/admin/system` 用于查看运行维护信息。
...
优先在后台 `/admin/settings` 的“门店管理”中新增、编辑、启用或停用门店。停用门店不会从历史工单和历史排班中删除，只是不再出现在新建工单和新建排班选择里。

`config/stores.json` 仍保留为首次初始化和空表兜底，不建议作为日常维护入口。
...
优先在后台 `/admin/settings` 的“品牌管理”中新增、编辑、启用或停用品牌。停用品牌不影响历史工单展示，只会从新提交入口隐藏。

`config/brands.json` 仍保留为首次初始化和空表兜底，不建议作为日常维护入口。

门店提报页会把启用品牌显示为可多选选项；未在列表中的品牌仍可在“其他品牌”中手动填写。系统会继续把品牌合并保存到原有工单字段，同时写入多品牌关联表，便于详情、查询和导出统一展示。

### 如何维护节假日

优先在后台 `/admin/settings` 的“节假日管理”中维护日期、名称、类型和启用状态。排班日历会区分周末、法定节假日、调休日和公司特殊日；停用节假日后不再显示特殊样式。

`config/holidays.json` 仍支持 `{ "YYYY-MM-DD": "节假日名称" }` 这类旧格式，但只在 `holidays` 表为空时作为兜底读取。
...
项目级完成定义见 `docs/definition_of_done.md`，修复报告统一使用 `docs/fix_report_template.md`。

1. UI/交互问题必须以人工真实浏览器确认为最终准绳。
2. Codex 无法浏览器确认时，不得宣称 UI 已修复，只能报告“代码层已调整，待人工确认”。
3. 真实页面人工不通过时，即使自动化测试通过，也不能提交。
4. 所有破坏性操作必须先 `backup`、再 `dry-run`、最后使用明确 `confirm` 参数执行。
